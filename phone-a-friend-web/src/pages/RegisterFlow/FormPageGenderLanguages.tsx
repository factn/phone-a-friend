...
const basicInfoSchema = Yup.object().shape({
  gender: Yup.string().oneOf(['Male', 'Female'], 'Must select a gender').required('Gender is required'),
  languages: Yup.string().required('Languages is required'),
});

const FormPageGenderLanguages: React.FC<FormPage<UserFormPageTwo>> = ({ onSubmit, initialValues }) => {
      ...
      {({ submitForm }) => (
        <>
          <div style={{ marginBottom: 12 }}>
            <label style={{ display: 'block', marginBottom: 6 }}>How do you identify your gender?</label>
            <Field
              name="gender"
